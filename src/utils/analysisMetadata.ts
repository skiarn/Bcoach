...
function normalizeSpeedFactor(value: unknown): 1.5 | 2 | 4 | undefined {
  if (value === 1.5 || value === 2 || value === 4) {
    return value;
  }
...
function normalizeFeedbackPins(pins: unknown): FeedbackPin[] {
  ...
  return pins
    .filter(
      (pin): pin is Partial<FeedbackPin> =>
        Boolean(pin) && typeof pin === "object",
    )
    .map((pin, index) => {
...
function normalizeAnalysisSegments(segments: unknown): AnalysisSegment[] {
  ...
  return segments
    .filter(
      (segment): segment is Partial<AnalysisSegment> =>
    ...
    .map((segment, index) => {
      ...
      const attemptIndex =
        typeof segment.attemptIndex === "number" &&
        Number.isFinite(segment.attemptIndex)
...
function normalizePlaybackEdits(segments: unknown): PlaybackEditSegment[] {
  if (!Array.isArray(segments)) {
    return [];
  }
  ...
  return segments
    .filter(
      (segment): segment is Partial<PlaybackEditSegment> =>
        Boolean(segment) && typeof segment === "object",
    )
    .map((segment, index) => {
      const startTime = normalizeTimestamp(segment.startTime);
      ...
      return {
        id: normalizeString(segment.id) ?? `edit-${index + 1}`,
        startTime: normalizedStart,
        endTime: normalizedEnd,
        action,
        speedFactor:
          action === "speed"
            ? normalizeSpeedFactor(segment.speedFactor)
            : undefined,
      };
    })
...
function flattenSegmentsToLegacyLists(
  segments: AnalysisSegment[],
): Pick<EmbeddedAnalysisMetadataV3, "feedback" | "nextSteps"> {
...
function buildFallbackSegmentFromLegacy(
...
export function normalizeEmbeddedAnalysisMetadata(
  ...
  const normalizedSportId = normalizeSportId(
    metadata.sportId ?? metadata.skillType,
  );
  ...
  return {
    ...
    nextSteps:
      directNextSteps.length > 0 ? directNextSteps : legacyLists.nextSteps,
    shapes: Array.isArray(metadata.shapes) ? metadata.shapes : [],
    analysisSegments: normalizedSegments,
    playbackEdits: normalizePlaybackEdits(
      (metadata as EmbeddedAnalysisMetadataV3).playbackEdits,
    ),
  };
}

export function deriveLegacyFeedbackFromSegments(
  segments: AnalysisSegment[],
): string[] {
  return flattenSegmentsToLegacyLists(segments).feedback;
}

export function deriveLegacyNextStepsFromSegments(
  segments: AnalysisSegment[],
): string[] {
